...
function initializeSvg(): void {
  ...
  defsElt.append(makeTriangleMarker("hour-arrow", HOUR_HAND_WIDTH),
    makeTriangleMarker("minute-arrow", MINUTE_HAND_WIDTH));
...
function intializeClock(): void {
  ...
  for (let i = 0; i < NUM_HOURS; i++) {
    ...
    mark.setAttribute("y1", HOUR_MARKS_OUTER as any);
    ...
    if (i % HOUR_M1_PERIOD) {
      mark.setAttribute("y2", M0_HOUR_MARKS_INNER as any);
      mark.setAttribute("stroke-width", M0_HOUR_MARKS_WIDTH as any);

      label.setAttribute("font-size", M0_HOUR_LABEL_SIZE as any);
      label.setAttribute("y",
        (M0_HOUR_LABEL_R * Math.cos(angleRadians)).toFixed(12) as any);
      label.setAttribute("x",
        (M0_HOUR_LABEL_R * -Math.sin(angleRadians)).toFixed(12) as any);
    } else {
      mark.setAttribute("y2", M1_HOUR_MARKS_INNER as any);
      mark.setAttribute("stroke-width", M1_HOUR_MARKS_WIDTH as any);

      label.setAttribute("font-size", M1_HOUR_LABEL_SIZE as any);
      label.setAttribute("y",
        (M1_HOUR_LABEL_R * Math.cos(angleRadians)).toFixed(12) as any);
      label.setAttribute("x",
        (M1_HOUR_LABEL_R * -Math.sin(angleRadians)).toFixed(12) as any);
    }
  ...
  for (let i = 0; i < NUM_MINUTES; i++) {
    ...
    mark.setAttribute("stroke", POS_COLOR);
    mark.setAttribute("y1", MINUTE_MARKS_INNER as any);
